...
impl TestCtx {
    fn new() -> Self {
        ...
        let client: ChannelContractClient<'static> =
...
fn test_initiate_dispute_sets_closing_status() {
    ...
    let agent_sig = sign_state(env, &ctx.agent_sk, &ctx.channel_id, iteration, agent_balance, server_balance);
    ...
    ctx.client.initiate_dispute(&ctx.channel_id, &state, &agent_sig, &true);

    // Channel is now Closing — close_channel (which requires Open) must fail.
    let server_sig = sign_state(env, &ctx.server_sk, &ctx.channel_id, iteration, agent_balance, server_balance);
    let result = ctx.client.try_close_channel(&ctx.channel_id, &state, &agent_sig, &server_sig);
    assert!(result.is_err(), "close_channel should fail on a Closing channel");
}
...
fn test_resolve_dispute_with_higher_iteration_then_finalize() {
    ...
    let server_sig_stale = sign_state(env, &ctx.server_sk, &ctx.channel_id, stale_iter, stale_agent_bal, stale_server_bal);
    ...
    ctx.client.initiate_dispute(&ctx.channel_id, &stale_state, &server_sig_stale, &false);
    ...
    let agent_sig = sign_state(env, &ctx.agent_sk, &ctx.channel_id, final_iter, final_agent_bal, final_server_bal);
    let server_sig = sign_state(env, &ctx.server_sk, &ctx.channel_id, final_iter, final_agent_bal, final_server_bal);
    ...
    ctx.client.resolve_dispute(&ctx.channel_id, &final_state, &agent_sig, &server_sig);
    ...
    assert_eq!(token_client.balance(&ctx.agent), 1_000_0000 - ctx.deposit + final_agent_bal);
...
fn test_finalize_before_window_panics() {
    ...
    assert!(result.is_err(), "finalize_dispute should fail while window is active");
}
...
fn test_resolve_requires_both_sigs() {
    ...
    let result = ctx.client.try_resolve_dispute(&ctx.channel_id, &state2, &agent_sig2, &bad_server_sig);
    assert!(result.is_err(), "resolve_dispute should fail with wrong server sig");
}
